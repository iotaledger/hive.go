...
func TestNew(t *testing.T) {
	...
	assert.Equal(t, 0, orderedMap.Size())
	_, _, existsHead := orderedMap.Head()
	_, _, existsTail := orderedMap.Tail()
	assert.False(t, existsHead)
	assert.False(t, existsTail)
}
...
func TestSetGetDelete(t *testing.T) {
	...
	assert.True(t, existsHead)
	assert.True(t, existsTail)
	assert.Equal(t, 1, orderedMap.Size())

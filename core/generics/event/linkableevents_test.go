...
var NewEvents = event.LinkableCollectionConstructor[Events](func(e *Events) {
	e.BlockMissing = event.Link(event.NewLinkable[int, Events](), e, func(t *Events) { e.BlockMissing.LinkTo(t.BlockMissing) })
	e.NestedEvents = event.Link(NewNestedEvents(), e, func(t *Events) { e.NestedEvents.LinkTo(t.NestedEvents) })
})
...
// NewNestedEvents contains the constructor of the NestedEvents object (it is generated by a generic factory).
var NewNestedEvents = event.LinkableCollectionConstructor[NestedEvents](func(e *NestedEvents) {
	e.BlockReceived = event.Link(event.NewLinkable[bool, Events](), e, func(t *NestedEvents) { e.BlockReceived.LinkTo(t.BlockReceived) })
})

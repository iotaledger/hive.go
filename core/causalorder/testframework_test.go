...
type TestFramework struct {
	...
	*CausalOrder[uint32, MockedEntityID, *MockedOrderedEntity]
}
...
func NewTestFramework(test *testing.T, workers *workerpool.Group, opts ...options.Option[TestFramework]) (newFramework *TestFramework) {
	return options.Apply(&TestFramework{
		...
		t.CausalOrder = New(
			t.Workers.CreatePool("CausalOrder", workerpool.WithWorkerCount(2)),
			...
			WithReferenceValidator[uint32, MockedEntityID](func(entity, parent *MockedOrderedEntity) (err error) {
				if entity.IsInvalid() {
				...
				return checkReference[uint32, MockedEntityID](entity, parent)
			}),
		)

		t.CreateEntity("Genesis", WithOrdered(true), WithIndex(0))
	})
}
...
func (t *TestFramework) EvictIndex(index uint32) {
